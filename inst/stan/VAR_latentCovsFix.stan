...
model {
  ...
  vector[N_obs] Ymus[sum(p_is_wcen)];
  vector[N] YB[n_p];
  ...
  for (pp in 1:N) {
    ...
    vector[D_cen] etaW_use[obs_id - maxLag];
    ...
    for(i in 1:n_p){
      if(p_is_wcen[i] == 1){
        ...
        if(is_SI[i] == 0){
          y_merge[i,pos_obs_p[i,1:n_obs_p[i]]] ~ normal(Ymus[p_is_wcen_pos[i],pos_obs_p[i,1:n_obs_p[i]]], sigmaW[i]);
        }
      }
    }
