data {
  ...
  int<lower=1> TP;
  int<lower=1> N_obs;
  int<lower=1> N_obs_id[N];
  int<lower=2> N_ind;
  vector[N_obs] y[N_ind];

  // conditional: 
  // 1. constant or random innovation variances?
  int logv_is_random;  
  // 2. individual parameters as outcome? 
  // int re_as_outcome;
  int n_cov;          // number of covariates - minimum of 1 for intercepts
  matrix[N,n_cov] W;  // predictors of individual parameters
  
  // 3. individual effect as predicors of time-invariant outcome
  ...
  int out_is_std;     // were outcome variables standardized? 
  
  // addon for missing 
  int<lower=0> N_miss[N_ind];   // has to be > 2 at the moment or next line fails 
  int pos_miss[N_ind,max(N_miss)];
} 
  ...
  if(n_out > 0) {
    for(nn in 1:(n_out_pred-n_out_pred_b)){
      ...
      b_pred[,nn] = (to_vector(b[,out_pred_which[1,nn]]) - 
                     mean(b[,out_pred_which[1,nn]]))/sd(b[,out_pred_which[1,nn]]);
      } else {
      b_pred[,nn] = to_vector(b[,out_pred_which[1,nn]]);
      }
    }
