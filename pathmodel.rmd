...
\begin{tikzpicture}[
    ...
    % draw nodes
    \foreach \i [remember = \i as \lasti (initially 1)] in {2, ..., 2}
    \node  [latent]  (y\i wt-1)  [below = 5em of y\lasti wt-1]  {$y_{\i ,t-1}^w$};\foreach \j [remember = \j as \lastj (initially 1)] in {2, ..., 2}
        \foreach \i [remember = \i as \lasti (initially 1)] in {1, ..., 2}
        \node  [latent]  (y\i wt-\j)  [left = 5em of y\i wt-\lastj]  {$y_{\i ,t-\j}^w$};\foreach \i [remember = \i as \lasti (initially 1)] in {2, ..., 2}
...
\draw  [path, postaction = random_cl, sloped, pos = .2]  (y1wt-2)  to node  []  {}  (y2wt);
...
\draw  [path, postaction = random_cl, sloped, pos = .2]  (y2wt-2)  to node  []  {}  (y1wt);
\draw  [path, postaction = random, anchor = west]  (y2wt-2.south)  |-  +(0em, -1em) -|  node  []  {$\phi_{22;(2)}$}  (y2wt.south);
\draw  [path, postaction = random]  (y1wt-2)  to node  []  {$\phi_{11;(1)}$}  (y1wt-1);
\draw  [path, postaction = random_cl, sloped, pos = .2]  (y1wt-2)  to node  []  {$\phi_{12;(1)}$}  (y2wt-1);
\draw  [path, postaction = random_cl, sloped, pos = .2]  (y2wt-2)  to node  []  {$\phi_{21;(1)}$}  (y1wt-1);
\draw  [path, postaction = random]  (y2wt-2)  to node  []  {$\phi_{22;(1)}$}  (y2wt-1);
...
\node  [coordinate]  (c12)  [above = 2em of lnsigma2_2]  {}; \draw  [path]  (c1)  to node  []  {}  (mu_1);
