VARmodelPaths <- function(VARmodel, data = NULL, labels = NULL) {
  ...
  if (VARmodel$q == 1) {
    ...
    \\node [latent] (mu_1) {$\\mu_{1,t}$};
    \\node [latent] (phi_1) [right = 1.5em of mu_1] {$\\phi_{1}$};
    \\node [latent] (lnsigma2_1) [right = 1.5em of phi_1] {$\\log(\\pi_{1})$};
    "
    # draw (co-)variances conditional on existing correlations
    if (nrow(model[model$Param == "r_mu_1.phi_11"]) > 0) {
      r_mu_1.phi_11 <- "\\draw [cov] (mu_1.north) to node [] {} (phi_1.north);"
    }
    if (nrow(model[model$Param == "r_mu_1.ln.sigma2_1"]) > 0) {
      r_mu_1.ln.sigma2_1 <- "\\draw [cov] (mu_1.north) to node [] {} (lnsigma2_1.north);"
    }
    if (nrow(model[model$Param == "r_phi_11.ln.sigma2_1"]) > 0) {
      r_phi_11.ln.sigma2_1 <- "\\draw [cov] (phi_1.north) to node [] {} (lnsigma2_1.north);"
    }
    covs <- paste(r_mu_1.phi_11, r_mu_1.ln.sigma2_1, r_phi_11.ln.sigma2_1, collapse = "")
  } else { # for q > 1
    ...
    for (i in 1:n_bpars) {
      if (i == 1) {
        bpars_vec <- c(
          bpars_vec, paste0(
            "\\node  [latent", ifelse(
              # draw in grey if parameter is fixed
        ...
        bpars_vec <- c(
          bpars_vec, paste0(
            ...
            all_bpars[i, "Param"], ")  [right = 1em of ", all_bpars[i - 1, "Param"],
            "]  {$\\", names_bpars[i], "$};\n"
          )
        )
      }
    ...
    # \\node  [latent]  (mu_y1)  [right = 1.5em of phi_y1]  {$log(\\pi_{y_1})$};
    #
    # "
    # paste("
    # % draw nodes
