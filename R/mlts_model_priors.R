...
mlts_model_priors <- function(VARmodel, default = F){
  ...
  if(default == T){
    ...
    VARmodel[VARmodel$Type=="RE correlation",cols] = data.frame("LKJ", 1, NA)
    ...
    VARmodel[VARmodel$Type=="RE prediction", cols] = data.frame("normal", 0, 10)
    VARmodel[VARmodel$Type=="Outcome prediction", cols] = data.frame("normal", 0, 10)
    VARmodel[VARmodel$Type=="Outcome prediction" &
             VARmodel$Param_Label == "Residual SD", cols] = data.frame("cauchy", 0, 2.5)
  }
